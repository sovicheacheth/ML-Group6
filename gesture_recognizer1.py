...
def getfiles(data_directory, filenames):    
    dir_files = {}
    for x in filenames:
        dir_files[x]=io.imread(join(data_directory, x))
    return dir_files
...
def train_binary(train_list, data_directory):
    ...
    for user in train_list:
        list_.append(pd.read_csv(join(data_directory, user , user + '_loc.csv'),index_col=None,header=0))
...
def get_data(user_list, img_dict, data_directory):
    ...
    for user in user_list:
        user_images = glob.glob(join(data_directory, user, '*.jpg'))

        boundingbox_df = pd.read_csv(join(data_directory, user, user + '_loc.csv'))
...
#    img = cv2.imread('C:/test/hands/V7.jpg')
#    result = gs.recognize_gesture(img)
